use gdbstub_arch::x86::reg::{F80, X86SegmentRegs, X86_64CoreRegs, X87FpuInternalRegs};
...
impl From<kvm_fpu> for Fpu {
	fn from(kvm_fpu: kvm_fpu) -> Self {
		...
		// TODO: Use array::map once stabilized
		let st = kvm_fpu
			.fpr
			.iter()
			.map(|fpr| fpr[..10].try_into().unwrap())
			.collect::<Vec<_>>()
			.try_into()
			.unwrap();
		...
		// TODO: Use array::map once stabilized
		let xmm = IntoIterator::into_iter(kvm_fpu.xmm)
			.map(u128::from_ne_bytes)
			.collect::<Vec<_>>()
			.try_into()
			.unwrap();
